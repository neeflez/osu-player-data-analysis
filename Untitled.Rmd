...
```{r}
cv <- function(x) {
  sd(x) / mean(x)  # Współczynnik zmienności
}

# Obliczanie współczynników zmienności dla wybranych zmiennych
wspolczynniki_zmiennosci <- sapply(osu_data[, c("celnosc", "liczba.zagran", "liczba.pp", "Ocena.SS", "Ocena.S", "Ocena.A", "ilosc.medali")], cv)

# Tworzenie ramki danych z wynikami
wspolczynniki_zmiennosci_df <- data.frame(
  zmienna = names(wspolczynniki_zmiennosci),
  wspolczynnik_zmiennosci = wspolczynniki_zmiennosci
)

# Wyświetlanie wyników
print(wspolczynniki_zmiennosci_df)
```