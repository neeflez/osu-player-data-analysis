...
# 3. Analiza skupień
Tak jak już wcześniej zauważyliśmy nasze dane można podzielić na graczy turniejowych oraz rankingowych. Sprawdźmy czy taki podział jest zasadny. Pomoże nam to odpowiedzieć na pytanie czy granie więcej prowadzi do lepszych wyników czy nie?

```{r}
kolumny_do_standaryzacji <- osu_data[, c("liczba.pp", "ilosc.medali", "celnosc", "liczba.zagran")]

# Zestandaryzuj wybrane kolumny
standaryzowane_dane <- scale(kolumny_do_standaryzacji)

# Przekształć macierz z zestandaryzowanymi danymi z powrotem do data frame
standaryzowane_dane_df <- as.data.frame(standaryzowane_dane)

# Dodaj nazwy kolumn do zestandaryzowanych danych
colnames(standaryzowane_dane_df) <- c("liczba.pp", "ilosc.medali", "celnosc", "liczba.zagran")

# Jeśli chcesz, możesz dołączyć zestandaryzowane dane z powrotem do oryginalnego data frame
osu_data <- cbind(osu_data, standaryzowane_dane_df)

# Wyświetl pierwsze wiersze zestandaryzowanych danych
head(osu_data)


# Funkcja do obliczania współczynnika zmienności
wspolczynnik_zmiennosci <- function(x) {
  sd(x) / mean(x)
}

# Zastosuj funkcję do wybranych kolumn
wspolczynniki <- sapply(osu_data[, c("liczba.pp", "ilosc.medali", "celnosc", "liczba.zagran")], wspolczynnik_zmiennosci)

# Stwórz data frame z wynikami
wynik_zmiennosci <- data.frame(zmienna = names(wspolczynniki), wspolczynnik_zmiennosci = wspolczynniki)
print(wynik_zmiennosci)

# Oblicz współczynniki korelacji
wspolczynniki_korelacji <- cor(osu_data[, c("liczba.pp", "ilosc.medali", "celnosc", "liczba.zagran")])

# Wyświetl macierz korelacji
print(wspolczynniki_korelacji)
```